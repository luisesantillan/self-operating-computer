...
async def get_next_action(model, messages, objective, session_id):
    ...
    if model == "gpt-4o-mini":
        return call_gpt_4o_mini(messages), None
    if model == "gpt-4o-mini-with-ocr":
        operation = await call_gpt_4o_mini_with_ocr(messages, objective, model)
    ...
    if model == "llava":
        ...
        return operation, None
    if model == "llava-phi":
        operation = call_ollama_llava_phi(messages)
        return operation, None
    if model == "claude-3":
...
async def call_gpt_4o_mini_with_ocr(messages, objective, model):
    if config.verbose:
        print("[call_gpt_4o_mini_with_ocr]")
    ...
    try:
        ...
        for operation in content:
            if operation.get("operation") == "click":
                ...
                if config.verbose:
                    print(
                        "[call_gpt_4o_mini_with_ocr][click] text_to_click",
                        text_to_click,
                    )
                ...
                if config.verbose:
                    print(
                        "[call_gpt_4o_mini_with_ocr][click] text_element_index",
                        text_element_index,
                    )
                    print(
                        "[call_gpt_4o_mini_with_ocr][click] coordinates",
                        coordinates,
                    )
                    print(
                        "[call_gpt_4o_mini_with_ocr][click] final operation",
                        operation,
                    )
...
def call_ollama_llava_phi(messages):
    if config.verbose:
        print("[call_ollama_llava_phi]")
    time.sleep(1)
    try:
        screenshots_dir = "screenshots"
        if not os.path.exists(screenshots_dir):
        ...
        if config.verbose:
            print(
                "[call_ollama_llava_phi] user_prompt",
                user_prompt,
            )
